...
class MotionDataset(torch.utils.data.Dataset):
    ...
    def is_frontal_camera(indices, sample_data):
        ...
        print("sensor['channel']:", sensor["channel"])
    ...
    def query_list(self, list, table_name, key):
        target = None
        # print(list)
        for i in list:
            if i[table_name].split("/")[1] == key and i["is_key_frame"] == True:
                target = i
                break
        if target is None:
            print("Caught Error!")
    ...
    def frontal_to_camera_filename(self, indices, frontal_image):
        token_list = [i['sample_token'] for i in frontal_image]

        # print(len(token_list))
        # TODO:
        all_images_list = [MotionDataset.query_range(
            indices, "sample_data", i, "sample_token") for i in token_list]
        camera_images_list = [[ self.query_list(i, "filename", j) for i in all_images_list] for j in self.camera_list]
        # print(camera_images_list[0])
    ...
    def load_or_generate_items(self, metadata_dir, dataset_name, split, tables, indices):
        ...
        for i in scene_frontal_frames:
            for j in self.fps_stride_tuples:
                for k in MotionDataset.enumerate_video_segments(i[1], self.sequence_length , j[0], j[1]):
                    # 检查 k 中是否有任何一个子项的字符串包含 "samples"
                    if any("samples" in item["filename"] for item in k):
                        self.items.append({"video": self.frontal_to_camera_filename(indices, k), "fps": j[0], "scene": i[0]})
    ...
    def __getitem__(self, index: int):
        ...
        if self.stage == 'train' and self.training_seq_len is not None:
            ...
            if selected_method=="random":
                ...
                if stride * self.training_seq_len > len(item['video'][0]):
                    raise ValueError("The product of stride and seq_len should not exceed the video length.")
                start = random.randint(0, max(stride - 1, len(item['video'][0]) - stride * self.training_seq_len))
            ...
            mask = [True] * len(item['video'][0])
        pts = [
            (i["timestamp"] - item["video"][0][0]["timestamp"] + 500) // 1000
            for i in item["video"][0]
        ]
        ...
        images = [
            [Image.open(io.BytesIO(self.reader.read(i["filename"])))
            for idx, i in enumerate(item["video"][j]) if mask[idx]] for j in range(len(self.camera_list))
        ]
        ...
        result = {
            ...
            # loader collation
            # "video": torch.stack([self.transforms(i) for i in images]).permute(1, 0, 2, 3),
            "video": torch.stack([torch.stack([self.transforms(i) for i in images[k]]) for k in range(len(self.camera_list))]).permute(0, 2, 1, 3, 4), # [V, T, C, H, W] -> [V, C, T, H, W]
        ...
        if self.additional_image_annotations is not None:
            for i in self.additional_image_annotations:
                ...
                result[i.name] = torch.stack(result[i.name])
        '''
        
        if self.additional_image_annotations is not None:
            for i in self.additional_image_annotations:
                result[i.name]  = []
                for k in range(len(self.camera_list)):
                    result_str = []
                    for j in item["video"][k]:                
                        if i.name == "bbox":
                            new_filename = j["filename"]
                        else: # 'hdmap'
                            new_filename = j["filename"].replace("samples","maps/samples")
                        if i.get(new_filename) is not None:
                            result_str.append(self.condition_transform(i.get(new_filename)))
                        else:
                            result_str.append(self.condition_transform(Image.new('RGB',(100,100), 'black')))
                    result[i.name].append(torch.stack(result_str))
                result[i.name] = torch.stack(result[i.name])
        ...
        #result["condition_images_visual"] = result["bbox"]+result["hdmap"]
