...
'''
def save_sample(x, save_path=None, fps=8, normalize=True, value_range=(0, 1), force_video=False, verbose=True, cfg=None):
    """
    Args:
    ...
    if not force_video and x.shape[1] == 1:  # T = 1: save as image
        ...
        if normalize:
            ...
            for i in range(0, total_H, H):
                segment = x[:, :, i:i+H, :]
                if segment.min() < 0 or segment.max() > 1:
                    # Normalize each segment
                    segment_min = segment.min()
                    segment_max = segment.max()
                    x[:, :, i:i+H, :] = (segment - segment_min) / (segment_max - segment_min)
        
        # normalize in H
        
        x = x.permute(1, 2, 3, 0)
        x = (x * 255).byte().to("cpu")
        write_video(save_path, x, fps=fps, video_codec="h264", options={'crf': '0'}) # 0: lossless, high quality
    if verbose:
...
def save_sample(x, save_path=None, fps=8, normalize=True, value_range=(-1, 1), force_video=False, verbose=True, cfg=None, divide=False, tag=None):
    """
    Args:
        x (Tensor): shape [C, T, H, W]
    """
    ...
    if not force_video and x.shape[1] == 1:  # T = 1: save as image
        ...
        if normalize:
            ...
            x_copy = x.clone()
            if divide and tag is not None:
                # Normalize specified region to (0, 1)
                specified_region = x_copy[:, :, tag[0]:tag[1], :].clamp(min=0, max=1)
                specified_region = specified_region.sub_(0).div_(1 - 0)

                # Normalize remaining part to (-1, 1)
                remaining_region = torch.cat((x_copy[:, :, :tag[0], :], x_copy[:, :, tag[1]:, :]), dim=2)
                remaining_region = remaining_region.clamp_(min=low, max=high)
                remaining_region = remaining_region.sub_(low).div_(max(high - low, 1e-5))

                # Concatenate the normalized parts back together
                x_copy = torch.cat((remaining_region[:, :, :tag[0], :], specified_region, remaining_region[:, :, tag[0]:, :]), dim=2)
            else:     
                x_copy = x_copy.clamp_(min=low, max=high)
                x_copy = x_copy.sub_(low).div_(max(high - low, 1e-5))

        
        x_copy = x_copy.permute(1, 2, 3, 0)
        x_copy = (x_copy * 255).byte().to("cpu")
        # write_video(save_path, x_copy, fps=fps, video_codec="libx264", options={'crf': '0'}) # 0: lossless, high quality
        
        write_video(save_path, x_copy, fps=fps, video_codec="libx264", options={"crf": "18", "pix_fmt": "yuv420p"})
