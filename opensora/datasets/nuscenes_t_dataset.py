...
class NuScenesTDataset(NuScenesDataset):
    ...
    def build_clips(self, data_infos, scene_tokens):
        ...
        #     len_scene.append(len(scene))
        # print("len_scene: ", len_scene) 
        # print("max: ", max(len_scene))
        scene_tokens_new = []
        filter_ = ['f9878012c3f6412184c294c13ba4bac3', 'c29f3669ab774a8296aba6d57427379a', 'f9878012c3f6412184c294c13ba4bac3', '2acc9fb43d5941d7bdd81c7d62834a6f']
        if self.filter_:
            for scene in scene_tokens:
                if scene[0] in filter_:
                    print(len(scene))
                    scene_tokens_new.append(scene)
            scene_tokens = scene_tokens_new 
        # ipdb.set_trace()
        
        if self.whole_scene:
            for scene in scene_tokens:
                clip = [self.token_data_dict[token]
                        for token in scene] # all tokens in scene
    ...
    def load_annotations(self, ann_file):
        ...
        '''
        data['infos'][0].keys()
        dict_keys(['lidar_path', 'token', 'sweeps', 'cams', 'lidar2ego_translation', 'lidar2ego_rotation', 'ego2global_translation', 'ego2global_rotation', 'timestamp', 'location', 'description', 'timeofday', 'is_key_frame', 
        'gt_boxes', 'gt_names', 'gt_velocity', 'num_lidar_pts', 'num_radar_pts', 'visibility', 'valid_flag'])
        '''
                    ...
                    for key, root_path in data_dict.items():
                        ...
                        for img_path in example["metas"].data['filename']:  
                            ...
                            if os.path.exists(new_filename) and self.annotation[key]:
                                ...
                                if key == 'hdmap' and len(result_str)!=0:
                                    result_str.append(result_str[-1]) # 前一帧 # TODO:
                                else:
                                    result_str.append(Image.new('RGB', (100, 100), 'black'))
