...
class STDiT3(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # 1. Define condition encoder

        layout_memory = nn.Parameter(torch.randn(1, 1152, 16, 16))
        ...
        for i in range(len((4, 8, 16, 16))-1):
            self.map_box_attn_blocks.append(
                BiMoT(
    ...
    def forward(
        ...
        **kwargs):
        ...
        x = rearrange(x, "B T S C -> B (T S) C", T=T, S=S)#S=H*W
        # === blocks ===
        #for spatial_block, temporal_block in zip(self.spatial_blocks, self.temporal_blocks):
        for idx, (spatial_block, temporal_block) in enumerate(zip(self.spatial_blocks, self.temporal_blocks)):
            
            if idx % 7 == 0 and self.enable_condition:
                x = rearrange(x,"B (T H W) C -> (B T) C H W",H=H,W=W,B=B,T=T)# S=H*W,[B, T, S, C] -> [B, T, H*W, C]->[B*T, C, H, W]
                
                # layout_condition in to patch
                # layout_conditions is B, T, C, H, W
                #patchify
                layout_conditions_patch = self.x_embedder(layout_conditions)#[B, T,C, H, W] -> [B, (T S), C]
                #rearrange
                layout_conditions_patch = rearrange(layout_conditions_patch, "B (T H W) C -> (B T) C H W",H=H,W=W,B=B,T=T)
                image_only_indicator = torch.zeros(B, T, dtype=x.dtype, device=x.device)
                x = self.map_box_attn_blocks[idx%7](x,layout_conditions_patch,image_only_indicator)# [B*T, C, H, W], [B*T, C, H, W], [B, T]
