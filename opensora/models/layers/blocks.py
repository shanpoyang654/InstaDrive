...
class Attention(nn.Module):
    def __init__(
        ...
        self.scale = self.head_dim**-0.5 * scale_compensation
...
class PositionEmbedding2D(nn.Module):
    ...
    def _get_cached_emb(
        ...
        use_separate_scales: bool = False,
    ):
        if use_separate_scales:
            # 2-dim pos_embedding
            grid_h = torch.arange(h, device=device) / scale[0] 
            grid_w = torch.arange(w, device=device) / scale[1] 
            if base_size is not None:
                grid_h *= base_size[0] / h 
                grid_w *= base_size[1] / w 
            # 2-dim pos_embedding
        else:
    ...
    def forward(
        ...
        h: int,
        w: int,
        scale: Optional[float] = 1.0, # Optional[Union[float, Tuple[float, float]]] = None
        base_size: Optional[int] = None, 
        use_separate_scales = False,
    ) -> torch.Tensor:
        return self._get_cached_emb(x.device, x.dtype, h, w, scale, base_size, use_separate_scales)




class CyclicPositionEmbedding2D(nn.Module):
    def __init__(self, dim: int) -> None:
        super().__init__()
        self.dim = dim
        assert dim % 4 == 0, "dim must be divisible by 4"
    ...
    def _get_cached_emb(
        ...
        if use_separate_scales:
            # 2-dim pos_embedding
            grid_h = torch.arange(h, device=device) / scale[0] 
            # TODO: 1-dim position encoding 
            # grid_w = torch.arange(w, device=device) / scale[1] 
            pos_angle = torch.linspace(0, 2 * math.pi, w, device=device) % (2 * math.pi)  
            cos_angle = torch.cos(pos_angle) * w / 2 + w / 2 # [0, W]
            sin_angle = torch.sin(pos_angle) * w / 2 + w / 2
            # TODO: 2-dim position encoding -> how to use?  replace adding
            grid_w = ((sin_angle + cos_angle) / 2) / scale[1] 
            ...
            cos_angle = torch.cos(pos_angle) * w / 2 + w / 2 # [0, W]
            sin_angle = torch.sin(pos_angle) * w / 2 + w / 2
            grid_w = ((sin_angle + cos_angle) / 2) / scale
            if base_size is not None:
                grid_h *= base_size / h
                grid_w *= base_size / w
...
def precompute_freqs_cis(dim: int, seq_len: int, theta: float = 10000.0, scale=1.0):
    # 计算词向量元素两两分组之后，每组元素对应的旋转角度
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 2)[: (dim // 2)].float() / dim))
    # 生成 token 序列索引 t = [0, 1,..., seq_len-1]
    t = torch.arange(seq_len, device=freqs.device) / scale
...
if __name__ == "__main__":
    # res = get_1d_sincos_pos_embed(4, 5)
    res = precompute_freqs_cis(4, 5)
    print(res)