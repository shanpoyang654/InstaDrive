...
# torchrun --master_port=29502 --nnodes=1 --nproc_per_node=${gpus} scripts/multi_train_vae_combine_encode.py ${config} --ckpt-path ${pretrain}
torchrun --master_port=29500 --nnodes=1 --nproc_per_node=${gpus} scripts/multi_train.py ${config} --ckpt-path ${pretrain}
