...
def save_imgs(x, label_str, dir, camera_list, img_path):
            ...
            for step, batch in pbar:
                ...
                for k, value in batch.items():
                    if k not in ["bbox", "hdmap", "traj", "img_path"]:
                        ...
                        V = traj_conditions.shape[1]
                        traj_conditions = torch.cat([traj_conditions[:,i] for i in range(V)], dim=4) # [B, T, C, H, V*W]
                        model_args["traj"] = traj_conditions
                    elif k == 'img_path':
                        ...
                        for loop_i in range(loop):
                            ...
                            if cfg.model.get("enable_condition", False):
                                ...
                                model_args["layout_conditions"] = model_args["hdmap"][:1,loop_i*cfg.num_frames:(loop_i+1)*cfg.num_frames].to(device, dtype)+\
                                    ...
                                    model_args["traj"][:1,loop_i*cfg.num_frames:(loop_i+1)*cfg.num_frames].to(device, dtype)# [B,V,T,C,H,W]
                                
                                if loop_i>0:
                                    model_args["layout_conditions"] = model_args["hdmap"][:1,loop_i*(cfg.num_frames-4):loop_i*(cfg.num_frames-4)+cfg.num_frames].to(device, dtype)+\
                                        model_args["bbox"][:1,loop_i*(cfg.num_frames-4):loop_i*(cfg.num_frames-4)+cfg.num_frames].to(device, dtype)+\
                                        model_args["traj"][:1,loop_i*(cfg.num_frames-4):loop_i*(cfg.num_frames-4)+cfg.num_frames].to(device, dtype)
                                
                                if cfg.model.get("condition_downsample", False):
                                    B, T, _, _, _ = model_args["layout_conditions"].shape
                                    # downsampled_layout = []
                                    # for i in range(V):
                                    downsampled = rearrange(model_args["layout_conditions"], 'B T C H W -> (B T) C H W', B=B, T=T)
                                    ...
                                    model_args["layout_conditions"] = downsampled
                                        ...
                                        for idx in range(len(video_clips[0])):
                                            ...
                                            if cfg.model.get("enable_condition", False):
                                                condition_images = model_args["hdmap"].to(device, dtype)[0].detach()+\
                                                                        ...
                                                                        model_args["traj"].to(device, dtype)[0].detach() # [T,C,H,6W]
                        ...
                        if coordinator.is_master():
                            ...
                            if cfg.model.get("enable_condition", False):                        
                                condition_images = model_args["hdmap"][:,:cfg.num_frames].to(device, dtype)[0].detach()+\
                                    ...
                                    model_args["traj"][:,:cfg.num_frames].to(device, dtype)[0].detach()# [T,C,H,W]
                                # condition_images = torch.cat([condition_images[i] for i in range(V)], dim=3) # [T,C,H,W*view]
                                condition_images = condition_images.permute(1,0,2,3) # [C,T,H,6W]
                                # b x c x t x h x w
                ...
                if cfg.model.get("enable_condition", False):
                    model_args["layout_conditions"] = model_args["bbox"].to(device, dtype)[:,:cfg.num_frames]\
                        ...
                        + model_args["traj"].to(device, dtype)[:,:cfg.num_frames]  
                    if cfg.model.get("condition_downsample", False):
                        downsampled = rearrange(model_args["layout_conditions"], 'B T C H W -> (B T) C H W', B=B, T=T)
                        downsampled = F.interpolate(downsampled, scale_factor=cfg.model.scale_factor, mode='nearest')
                        downsampled = rearrange(downsampled, '(B T) C H W -> B T C H W', B=B, T=T)  
